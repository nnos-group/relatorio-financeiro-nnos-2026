...
# Ler conteúdos
$matrizRaw = Get-Content $matrizFile -Raw -Encoding UTF8
$uvaRaw = Get-Content $uvaFile -Raw -Encoding UTF8

# Extrair partes do Matriz
$mHeaderIdx = $matrizRaw.IndexOf('<header class="relative overflow-hidden')
$mMainIdx = $matrizRaw.IndexOf('<main class="max-w-[1440px]')
$mFooterIdx = $matrizRaw.IndexOf('<footer class="bg-slate-950')
$mFooterEndIdx = $matrizRaw.IndexOf('</footer>', $mFooterIdx) + 9

$matrizHeader = $matrizRaw.Substring($mHeaderIdx, $mMainIdx - $mHeaderIdx)
$matrizMain = $matrizRaw.Substring($mMainIdx, $mFooterIdx - $mMainIdx)
$matrizFooter = $matrizRaw.Substring($mFooterIdx, $mFooterEndIdx - $mFooterIdx)

# Injetar botões de alternância e menu no Navbar do Matriz
$matrizNavReplacement = @"
...
"@
$matrizHeader = $matrizHeader -replace '<button onclick="logout\(\)"[\s\S]*?<\/button>', $matrizNavReplacement

# Extrair partes do UVA
$uHeaderIdx = $uvaRaw.IndexOf('<header class="relative overflow-hidden')
$uNavIdx = $uvaRaw.IndexOf('<nav class="sticky top-0')
$uMainIdx = $uvaRaw.IndexOf('<main class="max-w-[1440px]')
$uFooterIdx = $uvaRaw.IndexOf('<footer class="bg-transparent')
$uFooterEndIdx = $uvaRaw.IndexOf('</footer>', $uFooterIdx) + 9

$uvaHeaderOnly = $uvaRaw.Substring($uHeaderIdx, $uNavIdx - $uHeaderIdx)
$uvaMain = $uvaRaw.Substring($uMainIdx, $uFooterIdx - $uMainIdx)
$uvaFooter = $uvaRaw.Substring($uFooterIdx, $uFooterEndIdx - $uFooterIdx)
...
$uvaMain = $uvaMain -replace 'id="kpis"', 'id="kpis-uva"'
...
<div id="login-overlay" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/95 backdrop-blur-xl px-4">
  <div class="glass-card w-full max-w-md p-8 rounded-2xl border border-white/10 shadow-2xl relative overflow-hidden">
    ...
    <div class="text-center mb-8">
      <img src="$logoSrc" alt="NNÓS Logo" class="h-12 mx-auto mb-4 opacity-95"/>
  ...
  <div id="view-hub" class="min-h-screen flex flex-col justify-between py-12 px-6">
    <div class="max-w-[1280px] mx-auto w-full">
      ...
      <div class="text-center max-w-2xl mx-auto mb-12">
        <img src="$logoSrc" alt="NNÓS Logo" class="h-14 mx-auto mb-4 opacity-95"/>
...
[System.IO.File]::WriteAllText($outputIndex, $htmlHead, [System.Text.Encoding]::UTF8)
Write-Host "build_portal.ps1 executado com sucesso! index.html atualizado."
